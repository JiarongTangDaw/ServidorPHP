    ...
    <form action="funciones.php" id="formulario" name="formulario" method="post">
        ...
        <div id="principal">
            ...
            <div id="nuevo">
                ...
                <input type="number" id="metacritic0" min= '0' max='100' value="">
            ...
            <div id="listado">
                <?php
                    foreach($videojuegos as $plataforma){
                        ...
                        $plat = str_replace(' ','_',$plataformaNombre);
                        foreach($arrayJuegos as $juego){
                ?>
                <select name="plataforma<?=$juego -> video_juego_id . $plat?>" id="plataforma<?=$juego -> video_juego_id . $plat?>">
                <?php foreach($plataformas as $plataforma): ?>
                    <option value="<?= $plataforma ?>" <?= $plataforma == $plataformaNombre ? 'selected' : '' ?>><?= $plataforma?></option>
                <?php endforeach; ?>
                </select>
                    <input type="text" name="titulo<?=$juego -> video_juego_id . $plat?>" id="titulo<?=$juego -> video_juego_id . $plat?>" value="<?= $juego -> titulo ?>">
                    <input type="number" name="anio<?=$juego -> video_juego_id . $plat?>" id="anio<?=$juego -> video_juego_id . $plat?>" value="<?= $juego -> anio ?>">
                    <input type="text" name="metacritic<?=$juego -> video_juego_id . $plat?>" id="metacritic<?=$juego -> video_juego_id . $plat?>" value="<?= $juego -> metacritic ?>">
                ...
                <div class="botones">
                    <input type="button" value="MOD" onclick= "modificar('<?=$juego -> video_juego_id?>', '<?=$plataformaNombre?>');">
                    <input type="button" value="DEL" onclick= "eliminar('<?=$juego -> video_juego_id?>', '<?=$plataformaNombre?>');">
                </div>
                <?php
                        }}
                ?>
